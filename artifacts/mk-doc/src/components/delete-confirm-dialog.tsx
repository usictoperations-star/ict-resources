...
function DependentsWarning({ dependents }: { dependents: DependentCounts }) {
  ...
  return (
    <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30 px-3 py-2.5 flex gap-2">
      ...
      <div className="text-sm text-amber-800 dark:text-amber-300">
        ...
        {items.join(", ")} {items.length === 1 ? "is" : "are"} linked to this application and will lose their association.
      </div>
    </div>
  );
}
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription asChild>
            <div>
              ...
              {dependents && <DependentsWarning dependents={dependents} />}
            </div>
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
