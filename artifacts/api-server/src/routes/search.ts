...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const [apps, infra, dbs, domains, repos, docs] = await Promise.all([
      db.select().from(applicationsTable).where(or(ilike(applicationsTable.name, pattern), ilike(applicationsTable.description, pattern))).limit(10),
      db.select().from(infrastructureTable).where(or(ilike(infrastructureTable.name, pattern), ilike(infrastructureTable.type, pattern))).limit(10),
      db.select().from(databasesTable).where(or(ilike(databasesTable.name, pattern), ilike(databasesTable.type, pattern))).limit(10),
